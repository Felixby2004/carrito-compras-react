...
class ReporteController {
    ...
    async getDataOperacional(reporte) {
        switch (reporte) {
            ...
            default:
                throw new errorHandler_1.AppError('Reporte operacional no soportado', 400);
        }
    }
    async reporteOperacional(req, res, next) {
        try {
            ...
            switch (reporte) {
                case 'ordenes-periodo':
                    ...
                    break;
                case 'inventario-valorizado':
                    this.drawTable(doc, ['SKU', 'Producto', 'Categoria', 'Stock', 'Valor'], data.slice(0, 200).map((s) => [
                        s.producto?.sku || '-',
                        s.producto?.nombre || '-',
                ...
                case 'movimientos-periodo':
                    ...
                    break;
                case 'stock-bajo':
                    this.drawTable(doc, ['SKU', 'Producto', 'Stock', 'Minimo'], data.slice(0, 200).map((s) => [
                        s.producto?.sku || '-',
                        s.producto?.nombre || '-',
                        `${s.stock_fisico}`,
                        `${s.stock_minimo}`,
    ...
    async facturaIndividual(req, res, next) {
        try {
    ...
    async comprobanteSimplificado(req, res, next) {
        try {
            const ordenId = Number(req.params.ordenId);
