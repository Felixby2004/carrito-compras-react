...
const emitirActualizacionStock = (productoId, stockFisico, stockReservado = 0) => {
    index_1.io.emit('stock-actualizado', {
        productoId,
        stockFisico,
        stockReservado,
...
const ajusteInventarioSchema = zod_1.z.object({
    cantidad: zod_1.z.number().int().positive(),
    motivo: zod_1.z.string().min(5),
});
const crearMovimientoSchema = zod_1.z.object({
    producto_id: zod_1.z.number(),
    tipo_movimiento: zod_1.z.enum(['entrada', 'salida', 'ajuste', 'devolucion']),
    cantidad: zod_1.z.number().int().positive(),
    motivo: zod_1.z.string(),
    referencia_id: zod_1.z.number().optional(),
});
class InventarioController {
    ...
    async getStockProducto(req, res, next) {
        try {
            ...
            if (!stock) {
                throw new errorHandler_1.AppError('Stock no encontrado', 404);
            }
            if (!producto) {
    ...
    async registrarMovimiento(req, res, next) {
        try {
            ...
            if (!stock) {
                throw new errorHandler_1.AppError('Stock del producto no encontrado', 404);
            }
            const stockAntes = stock.stock_fisico;
            ...
            if (datos.tipo_movimiento === 'entrada') {
                ...
                if (stockAntes < datos.cantidad) {
                    throw new errorHandler_1.AppError('Stock insuficiente', 400);
                }
                stockDespues = stockAntes - datos.cantidad;
            }
    ...
    async getMovimientos(req, res, next) {
        try {
            ...
            if (desde || hasta) {
                ...
                if (desde)
                    where.fecha_movimiento.gte = new Date(desde);
                if (hasta) {
    ...
    async crearOrdenCompra(req, res, next) {
        try {
            ...
            for (const articulo of articulos) {
                ...
                if (!producto) {
                    throw new errorHandler_1.AppError(`Producto ${articulo.producto_id} no encontrado`, 404);
                }
    ...
    async getOrdenesCompra(req, res, next) {
        try {
    ...
    async recibirMercaderia(req, res, next) {
        try {
            ...
            if (!ordenCompra) {
                throw new errorHandler_1.AppError('Orden de compra no encontrada', 404);
            }
    ...
    async actualizarEstadoOrdenCompra(req, res, next) {
        try {
            const id = parseInt(req.params.id);
    ...
    async getCuentasPorPagar(req, res, next) {
        try {
            const { proveedor_id } = req.query;
            const where = { estado: { in: ['pendiente', 'aprobada', 'parcial'] } };
            if (proveedor_id)
        ...
        try {
            ...
            if (filtro) {
                where.OR = [
                    ...
                    { ruc: { contains: filtro, mode: 'insensitive' } },
    ...
    async createProveedor(req, res, next) {
        try {
            ...
            if (existente) {
                throw new errorHandler_1.AppError('Ya existe un proveedor con este RUC', 409);
            }
    ...
    async realizarAjuste(req, res, next) {
        try {
            ...
            if (!producto_id || cantidad === undefined || !tipo || !motivo) {
                throw new errorHandler_1.AppError('Faltan campos requeridos', 400);
            }
            if (typeof cantidad !== 'number' || cantidad === 0) {
                throw new errorHandler_1.AppError('La cantidad debe ser un número diferente de 0', 400);
            }
            // Obtener stock actual
            const stock = await prisma.inv_stock_producto.findUnique({
                where: { producto_id },
            ...
            if (tipo === 'positivo') {
                ...
                if (stockAntes < Math.abs(cantidad)) {
                    throw new errorHandler_1.AppError('Stock insuficiente para hacer ajuste negativo', 400);
                }
                stockDespues = stockAntes - Math.abs(cantidad);
            }
            else {
                throw new errorHandler_1.AppError('Tipo de ajuste inválido (positivo/negativo)', 400);
            }
            // Actualizar stock
            const stockActualizado = await prisma.inv_stock_producto.update({
                where: { producto_id },
