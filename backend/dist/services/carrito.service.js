...
class CarritoService {
    ...
    async getCarrito(usuarioId, sessionId) {
        ...
        const subtotal = carrito.items.reduce((sum, item) => sum + (Number(item.precio_unitario) * item.cantidad), 0);
        const impuesto = this.calcularImpuesto(subtotal);
        const total = subtotal + impuesto;
        // Mapear items con stock disponible
        const itemsConStock = carrito.items.map((item) => {
    ...
    async addToCart(usuarioId, sessionId, data) {
        ...
        if (stockDisponible < data.cantidad) {
            throw new errorHandler_1.AppError(`Stock insuficiente. Solo hay ${stockDisponible} unidades disponibles.`, 400);
        }
        ...
        const itemExistente = carrito.items.find((item) => item.producto_id === data.producto_id);
        if (itemExistente) {
            const nuevaCantidad = itemExistente.cantidad + data.cantidad;
            // Validar stock nuevamente para la suma
            if (nuevaCantidad > stockDisponible) {
                throw new errorHandler_1.AppError(`No puedes agregar más. Stock disponible: ${stockDisponible}`, 400);
            }
    ...
    async updateCartItem(usuarioId, sessionId, itemId, cantidad) {
        ...
        if (usuarioId && item.carrito.usuario_id !== usuarioId) {
            throw new errorHandler_1.AppError('No autorizado', 403);
        }
        ...
        if (cantidad < 0) {
            throw new errorHandler_1.AppError('La cantidad no puede ser negativa', 400);
        }
        if (cantidad === 0) {
    ...
    async removeCartItem(usuarioId, sessionId, itemId) {
        const item = await prisma.ord_items_carrito.findUnique({
            where: { id: itemId },
    ...
    async clearCart(usuarioId, sessionId) {
        let carrito = null;
        if (usuarioId) {
    ...
    async mergeCart(usuarioId, sessionId) {
        ...
        if (carritoInvitado) {
            ...
            for (const item of carritoInvitado.items) {
                const existingItem = carritoUsuario.items.find((i) => i.producto_id === item.producto_id);
                if (existingItem) {
                    await prisma.ord_items_carrito.update({
