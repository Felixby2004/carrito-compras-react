...
export class DashboardController {
  ...
  private normalizarEstado(estado: unknown) {
  ...
  async getResumenGeneral(req: Request, res: Response, next: NextFunction) {
    try {
      const [
        totalOrdenes,
        totalVentas,
  ...
  async getProductosMasVendidos(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getEstadoInventario(req: Request, res: Response, next: NextFunction) {
    try {
      const [
        totalProductos,
        productosActivos,
  ...
  async getMovimientosInventario(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getClientesRecientes(req: Request, res: Response, next: NextFunction) {
    try {
      const { limite = 10 } = req.query;
  ...
  async getMetricasRendimiento(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getAdvancedAnalytics(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const productoCantidadMap = new Map<string, { nombre: string; cantidad: number; ingreso: number; costo: number }>();
      const estadoMap = new Map<string, number>();
      const hourlyMap = new Map<string, number>();
      const weekdayMap = new Map<string, number>();
      const monthlyMap = new Map<string, { ingresos: number; costos: number; ordenes: number }>();
      const abandonmentByDay = new Map<string, { carritos: number; ordenes: number }>();
      const descuentoCorr: Array<{ descuento: number; cantidad: number }> = [];
      ...
      const firstMonthByClient = new Map<number, string>();
      const cohortMap = new Map<string, Map<string, number>>();
      for (const o of cohortOrders) {
        const m = this.monthKey(new Date(o.created_at));
        if (!firstMonthByClient.has(o.cliente_id)) firstMonthByClient.set(o.cliente_id, m);
        const cohort = firstMonthByClient.get(o.cliente_id)!;
        const active = cohortMap.get(cohort) || new Map<string, number>();
      ...
      const ticketPorSegmento = new Map<string, number[]>();
