...
const crearOrdenSchema = z.object({
  ...
  identificacion: z.object({
    tipo: z.enum(['invitado', 'login', 'registro', 'autenticado']),
...
export class OrdenController {
    ...
    try {
      ...
      if (!usuarioId) {
        if (data.identificacion.tipo === 'autenticado') {
          ...
          if (!usuario) {
            throw new AppError('Credenciales inválidas: El correo no existe. Si no tiene cuenta, elija "Registrarme".', 401);
          }
          
          if (!data.identificacion.password) {
            throw new AppError('Se requiere contraseña para iniciar sesión. Por favor, ingrésela o vuelva a loguearse.', 401);
          }
  ...
  async getMisOrdenes(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const conditions: any[] = [{ created_by: req.user.id }];
      if (cliente) {
  ...
  async getMiOrdenById(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const userConditions: any[] = [{ created_by: req.user.id }];
      if (cliente) userConditions.push({ cliente_id: cliente.id });
